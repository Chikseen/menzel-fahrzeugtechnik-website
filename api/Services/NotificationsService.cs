...
public class NotificationsService
{
    ...
    public List<Notification> getAllNotifications()
    {
        ...
        for (int i = 0; i < data.Count; i++)
        {
            notificationsList.Add(new Notification(Int32.Parse(data[i][0]), DateTime.Parse(data[i][1]), DateTime.Parse(data[i][2]), data[i][3], data[i][4], data[i][5], bool.Parse(data[i][6]), data[i][7]));
        }
